...
def test_parse_log_content():
    ...
    with log_file() as f:
        ...
        assert '/api/v2/banner/25019354', [0.39] in result['log_entries'].items()
        assert '/api/1/photogenic_banners/list/?server_name=WIN7RB4', [0.133] in result['log_entries'].items()
...
def test_process_log():
    ...
    with mock.patch("gzip.open", mock.mock_open(read_data="\n".join(log_lines))) as mock_gzip_open:
